...
export function CommandBar() {
    ...
    return (<div className="flex h-[52px] items-center gap-3 border-b border-border px-4 bg-surface/50 flex-shrink-0 backdrop-blur-sm">
      {/* Search input */}
      <motion.div className={`
        ...
        `} animate={focused
            ...
            : { boxShadow: '0 0 0 0px transparent, 0 0 0px 0px transparent' }} transition={{ duration: 0.2 }}>
        <Search size={13} className={`flex-shrink-0 transition-colors ${focused ? 'text-primary' : 'text-muted-foreground'}`}/>
        <input ref={inputRef} value={searchQuery} onChange={(e) => handleSearch(e.target.value)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} placeholder="Search your photos..." className="flex-1 bg-transparent text-[13px] text-foreground placeholder:text-muted-foreground/70 outline-none"/>
      ...
      <div className="flex items-center gap-2">
        {/* Indexing */}
        {indexingStatus.isIndexing && (<div className="flex items-center gap-1.5 rounded-full bg-success/10 px-2.5 py-1">
