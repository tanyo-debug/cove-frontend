...
export function IndexingPanel() {
    ...
    useEffect(() => {
        getIndexingStatus().then((status) => setIndexingStatus(status));
        const interval = setInterval(() => {
            setIndexingStatus({
                progress: Math.min(100, indexingStatus.progress + Math.random() * 1.5),
                processed: Math.min(indexingStatus.total, indexingStatus.processed + Math.floor(Math.random() * 400)),
            });
        }, 2500);
        return () => clearInterval(interval);
    }, []);
    const currentStageIdx = stages.findIndex((s) => s.id === indexingStatus.stage);
    return (<div className="h-full overflow-auto p-6 max-w-xl mx-auto">
      <div className="mb-8">
      ...
      {/* Main progress */}
      <div className="mb-10 rounded-xl bg-card border border-border p-5">
        ...
        }} animate={{ width: `${indexingStatus.progress}%` }} transition={{ duration: 1, ease: 'easeOut' }}/>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {stages.map((stage, i) => {
            ...
            return (<motion.div key={stage.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }} className={`
              ...
              <div className={`
                  ...
                  ${isPending ? 'bg-muted' : ''}
                `}>
                {isComplete ? (<CheckCircle2 size={16} className="text-success"/>) : (<stage.icon size={16} className={`
