    ...
    <div className="h-full overflow-auto flex flex-col">
      ...
      {!hasResults && !loading && !showEmpty && (
        <motion.div
          ...
          className="flex flex-col items-center justify-center pt-32 pb-8 px-4"
        >
          <motion.div
            className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center mb-5"
            animate={{ y: [0, -3, 0] }}
          ...
          <div className="flex flex-wrap gap-1.5 justify-center max-w-md">
            {SUGGESTIONS.map((s, i) => (
              <motion.button
                ...
                className="rounded-full border border-border bg-card hover:bg-surface-hover hover:border-primary/30 px-3 py-1.5 text-[11px] text-secondary-foreground transition-all duration-200"
              >
                {s}
              </motion.button>
            ))}
