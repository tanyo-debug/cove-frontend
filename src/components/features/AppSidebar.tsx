...
import {
  Images, Users, Search, Activity, Settings, ChevronsLeft, ChevronsRight, Sparkles,
} from 'lucide-react';
import { useAppStore } from '@/store/useAppStore';

const navItems = [
  { id: 'library' as const, label: 'Library', icon: Images },
              ...
              <AnimatePresence mode="wait">
                {!collapsed && (
                  <motion.span
                    ...
                    className="relative z-10 truncate whitespace-nowrap font-medium"
                  >
                    {item.label}
                  </motion.span>
                )}
...
function IndexingMini({ collapsed }: { collapsed: boolean }) {
  const { isIndexing, progress, stage } = useAppStore((s) => s.indexingStatus);
  if (!isIndexing) return null;

  return (
    <div className="mx-2 mb-1 rounded-md bg-sidebar-accent/50 p-2">
